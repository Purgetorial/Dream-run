...
---------------------------- Run timer ----------------------------
-- Find this event handler in RunService.lua
ReqRunStart.OnServerEvent:Connect(function(plr)
	...
	-- ADD THESE TWO LINES
	print("Setting player team to Stage " .. data.CurrentStage)
	plr.Team = game:GetService("Teams"):FindFirstChild("Stage " .. data.CurrentStage)
...
local function onFinishPadTouched(hit, stageNumber)
	...
	if stageNumber < TOTAL_STAGES then
		...
		-- ADD THIS LINE to change the player's team
		plr.Team = game:GetService("Teams"):FindFirstChild("Stage " .. data.CurrentStage)
